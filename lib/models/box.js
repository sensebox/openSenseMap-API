...
boxSchema.index({ 'sensors._id': 1, 'sensors.lastMeasurement.createdAt': -1 });
...
boxSchema.statics.saveMeasurement = function (measurement) {
  return Promise.all([
    Measurement.create(measurement),
    // Model.update
    this.update(
      // only update if the lastMeasurement is before the inserted measurement
      {
        sensors: {
          '$elemMatch': {
            _id: measurement.sensor_id,
            '$or': [
              { 'lastMeasurement.createdAt': { $lt: measurement.createdAt } },
              { lastMeasurement: { '$exists': false } }
            ]
          }
        }
      },
      {
        'sensors.$.lastMeasurement': { createdAt: measurement.createdAt, value: measurement.value }
      }
    )
  ])
  .then(function (results) {
    const [ measurementCreated, updateResult ] = results;

    let errors;
    if (typeof measurementCreated.errors !== 'undefined') {
      errors = measurementCreated.errors;
    }
...
boxSchema.methods.saveMeasurementsArray = function (measurements) {
  ...
  const sensorsObj = box.sensors.reduce(function (sensorsObj, sensor) {
    const id = sensor._id.toString();
    if (!sensorsObj[id]) {
      sensorsObj[id] = { doc: sensor, lastMeasurement: sensor.lastMeasurement };
    }
  ...
  for (const measurement of measurements) {
    if (!sensorsObj[measurement.sensor_id]) {
      return Promise.reject(`measurement for sensor with id ${measurement.sensor_id} does not belong to box`);
    }

    if (!sensorsObj[measurement.sensor_id].lastMeasurement || measurement.createdAt > sensorsObj[measurement.sensor_id].lastMeasurement.createdAt) {
      const lastMeasurement = { createdAt: measurement.createdAt, value: measurement.value };
      sensorsObj[measurement.sensor_id].lastMeasurement = lastMeasurement;
      sensorsObj[measurement.sensor_id].doc.set('lastMeasurement', lastMeasurement);
    }
  }
