...
boxSchema.statics.findBoxById = function findBoxById (id, { lean = true, populate = true, includeSecrets = false, format, onlyLastMeasurements = false } = {}) {
  let populationProps = {};
  if (populate) {
  ...
  let findPromise = this.findById(id, populationProps);
  ...
  for (let i = box.sensors.length - 1; i >= 0; i--) {
    if (box.sensors[i]._id.equals(measurement.sensor_id)) {
      ...
      // only update lastMeasurement, if timestamp is actually the newest.
        .then(function ([m, b]) { // m === measurement, b === box
          if (!b.sensors[i].lastMeasurement || m.createdAt.valueOf() > b.sensors[i].lastMeasurement.createdAt.getTime()) {
...
boxSchema.statics.getBoxLocations = function getBoxLocations ({ format, boxId, fromDate, toDate }) {
  return this.findById(boxId, { locations: 1 })
    .lean()
    .then(function (box) {
      if (!box) {
        throw new ModelError('Box not found', { type: 'NotFoundError' });
