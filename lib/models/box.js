...
boxSchema.statics.initNew = function (req) {
  ...
  if (req.params.ttn) {
    const { app_id, dev_id, port, profile, decodeOptions } = req.params.ttn;
    boxData.integrations.ttn = { app_id, dev_id, port, profile, decodeOptions };
  }
...
boxSchema.methods.sendNewSketchMail = function sendNewSketchMail () {
  const box = this;
  User.findOne({ boxes: { '$in': [box._id] } })
    .then(function (user) {
      mails.sendNewSketchMail(user, box);
    });
};
