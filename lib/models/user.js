...
const { model: Box, schema: boxSchema } = box;
...
userSchema.set('toJSON', {
  ...
  transform: function transform (doc, ret) {
    const { firstname, lastname, email, role, language, boxes } = ret;

    return { firstname, lastname, email, role, language, boxes };
  }
});
...
userSchema.methods.removeBox = function removeBox (boxId) {
  ...
  const userOwnsBox = user.boxes.some(b => b._id.equals(boxId));
