...
const { config } = utils;
const { jwt_algorithm, jwt_secret, jwt_issuer, password_min_length, password_salt_factor, jwt_validity_ms, refresh_token_validity_ms } = config;
  ...
  return new Promise(function (resolve, reject) {
    jwt.sign(payload, jwt_secret, signOptions, (err, token) => {
      ...
      user.update({
        ...
        .exec()
        .then(function () {
          return resolve({ jwt: token, refreshToken });
        })
        .catch(function (err) {
          return reject(err);
        });
