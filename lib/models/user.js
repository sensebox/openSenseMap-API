...
userSchema.path('unconfirmedEmail').validate({
  ...
  validator: function (value, callback) {
    ...
    if (this._newEmail) {
      const user = this;
      if (!emailValidRegex.test(user._newEmail)) {
    ...
    .then(function (savedBox) {
      ...
      return user.save()
        .then(function () {
          ...
          postToSlack(`Eine neue <https://opensensemap.org/explore/${savedBox._id}|senseBox> Modell "${savedBox.model}" wurde registriert (${savedBox.name})`);
