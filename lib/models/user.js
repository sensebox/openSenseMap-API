...
const { mongoose } = require('../db'),
  ...
  { config: { jwt_algorithm, jwt_secret, jwt_issuer, password_min_length, password_salt_factor, jwt_validity_ms, refresh_token_validity_ms }, Honeybadger, postToSlack } = require('../utils'),
  uuid = require('uuid'),
...
const passwordHasher = function passwordHasher (plaintextPassword) {
  ...
  return bcrypt.hash(hashed, password_salt_factor); // signature <String, Number> generates a salt and hashes in one step
};
        ...
        .then(function (savedBox) {
          ...
          return user.save()
            .then(function () {
              ...
              postToSlack(`Eine neue <https://opensensemap.org/explore/${newBox._id}|senseBox> Modell "${savedBox.model}" wurde registriert (${savedBox.name})`);
...
userSchema.methods.destroyUser = function destroyUser (req) {
  ...
  return user
    ...
    .then(function (userWithBoxes) {
      ...
      userWithBoxes.mail('deleteUser', userWithBoxes.boxes);
      // delete the boxes..
