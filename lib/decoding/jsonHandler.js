...
let handleJSONObjectMultipleValues = function (obj) {
  let outArray = [];

  // transform into Array
  for (let sensorId of Object.keys(obj)) {
    // construct measurement
    let measurement = { sensor_id: sensorId };
    ...
    if (message) {
      ...
      if (typeof json !== 'undefined') {
        ...
        if (result && result[0]) {
          ...
          decodedJson = transformAndValidateArray(decodedJson);
