...
const castParam = function castParam (param, paramDataType, dataTypeIsArray) {
  ...
  let parser, check, expectCheckResult;
  switch (paramDataType) {
  case 'Number':
    parser = Number.parseFloat;
    check = Number.isNaN;
    expectCheckResult = true;
    break;
  case 'ISO8601':
    parser = parseTimestamp;
    check = timeIsValid;
    expectCheckResult = false;
    break;
  case 'String':
    parser = function (s) { return s.toString().trim(); };
  ...
  if (parser && check && typeof expectCheckResult !== 'undefined') {
    for (let i = 0, len = param.length; i < len; i++) {
      param[i] = parser(param[i]);
      if (check(param[i]) === expectCheckResult) {
        return;
      }
    }
  }
...
const retrieveParameters = function retrieveParameters (parameters = []) {
  return function (req, res, next) {
    for (const { name, aliases, dataType = 'String', allowedValues, mapping, required = false, defaultValue } of parameters) {
      ...
      case CAST_FAILED:
        return next(new restify.UnprocessableEntityError(`Parameter ${nameUsed} is not parseable as datatype ${(dataTypeIsArray ? dataType[0] : dataType)}`));
      case ILLEGAL_VALUE:
