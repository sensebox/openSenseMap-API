...
const retrieveParameters = function retrieveParameters (parameters = []) {
  return function (req, res, next) {
    for (const { name, aliases, dataType = 'String', allowedValues, mapping, required = false, defaultValue } of parameters) {
      ...
      // check if the parameter was found and is required
      if (required === true && typeof value === 'undefined') {
        ...
        // no value an not required, skip to the next parameter
        continue;
      } else if (typeof value === 'undefined' && typeof defaultValue !== 'undefined') {
        // there was no user defined value but a default value
