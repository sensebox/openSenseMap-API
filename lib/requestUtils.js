...
const retrieveLocationParameter = function ({ required } = { required: false }) {
  return function (req, res, next) {
    if (typeof req.params.location === 'undefined') {
      if (required) {
        return next(new restify.BadRequestError('missing required parameter location'));
      }
    ...
    try {
      const validLoc = transformAndValidateCoords(req.params.location);
      setUserParam(req, 'location', validLoc);
    } catch (err) {
      return next(new restify.UnprocessableEntityError(`invalid location: ${err}`));
    }
