...
const initRoutes = function initRoutes (server) {
  ...
  server.post({ path: `${config.userPath}/request-password-reset` }, usersController.requestResetPassword); // send password reset token

  server.post({ path: `${config.userPath}/password-reset` }, usersController.resetPassword); // submit new password with reset token as auth

  server.post({ path: `${config.userPath}/confirm-email` }, usersController.confirmEmailAddress);

  // check username and password
  server.post({ path: `${config.userPath}/sign-in` }, authHelpers.checkUsernamePassword, usersController.signIn);

  // Secured (needs authorization through jwt)
  ...
  server.post({ path: `${config.userPath}/sign-out` }, usersController.signOut);
