...
const initRoutes = function initRoutes (server) {
  ...
  // GET
  server.get({ path: config.basePath, version: '0.0.1' }, [ cache('2 minutes'), boxesController.findAllBoxes ]);
  ...
  server.get({ path: '/stats', version: '0.1.0' }, [ cache('2 minutes'), statisticsController.getStatistics ]);
