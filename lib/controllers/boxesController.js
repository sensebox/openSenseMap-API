...
const updateBox = function updateBox (req, res, next) {
  ...
  const { mqtt, ttn, loc, image, sensors, boxId } = req._userParams;

  Box.findById(boxId).then(function (box) {
    // easy string properties
    for (const prop of ['name', 'exposure', 'grouptag', 'description']) {
      if (req._userParams[prop]) {
    ...
    if (ttn) {
      const { app_id, dev_id, profile, decodeOptions } = ttn;
      box.set('integrations.ttn', { app_id, dev_id, profile, decodeOptions });
    }
    ...
    if (image) {
      const imageBuffer = requestUtils.decodeBase64Image(image);
 ...
 * @apiDescription Posts a new measurement to a specific sensor of a box. The field `lastMeasurement` is only updated, if the new measurement is newer than the previous lastMeasurement.
 * @apiVersion 0.0.1
 * @apiGroup Measurements
 ...
 * @apiUse SensorIdParam
 * @apiUse ContentTypeJSON
 * @apiParam (RequestBody) {Float} value the measured value of the sensor. Also accepts floats in strings.
...
const postNewMeasurement = function postNewMeasurement (req, res, next) {
  const { sensorId, value, createdAt } = req.params;

  jsonHandler.decodeMessage([{
    sensor_id: sensorId,
    value,
    createdAt
  }])
    ...
    .then(function (errors) {
      if (errors && errors.boxUpdateError === true) {
        return next(new restify.InternalServerError('Error updating lastMeasurement'));
      }
      ...
      return res.send(201, 'Measurement saved in box');
    })
    .catch(function (err) {
      let msg;
      if (err) {
 ...
 *
 * The `lastMeasurement` field of the sensors belonging to this box will only be updated with measurements newer than the previous last measurement.
 *
 * @apiVersion 0.1.0
 * @apiGroup Measurements
...
const postNewBox = function postNewBox (req, res, next) {
  req.user.addBox(req)
    .then(function (newBox) {
      ...
      requestUtils.clearCache(['findAllBoxes', 'getStats']);
    })
    .catch(function (err) {
      if (err.errors) {
...
module.exports = {
  ...
  updateBox: [
    ...
    requestUtils.retrieveParameter('name', 'String', false),
    requestUtils.retrieveParameter('grouptag', 'String', false),
    requestUtils.retrieveParameter('description', 'String', false),
    requestUtils.retrieveParameter('image', 'String', false),
    requestUtils.retrieveParameter('exposure', 'String', false, ['indoor', 'outdoor']),
    requestUtils.retrieveParameter('mqtt', 'object', false),
    requestUtils.retrieveParameter('ttn', 'object', false),
    requestUtils.retrieveParameter('loc', 'object', false),
    requestUtils.retrieveParameter('sensors', 'Array', false),
    updateBox
  ],
  getMeasurements,
  postNewMeasurement: [
  ...
  postNewMeasurements: [
    requestUtils.retrieveParameter('luftdaten', 'String'),
    postNewMeasurements
  ],
  postNewBox: [
    requestUtils.checkContentType,
  ...
  findAllBoxes: [
    requestUtils.retrieveParameter('exposure', 'String', false, ['indoor', 'outdoor']),
    requestUtils.retrieveParameter('model', 'Array,String', false),
    requestUtils.retrieveParameter('grouptag', 'Array,String', false),
    ...
    requestUtils.addCache('5 minutes', 'findAllBoxes'),
    findAllBoxes
  ]
};
