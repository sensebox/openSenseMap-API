...
const getData = function getData (req, res, next) {
  // default to now
  const toDate = requestUtils.parseTimeParameter(req, next, 'to-date', moment());
  if (!moment.isMoment(toDate)) {
    return next(toDate);
  }

  // default to 48 hours earlier from to-date
  const fromDate = requestUtils.parseTimeParameter(req, next, 'from-date', toDate.clone().subtract(48, 'hours'));
...
const getDataMulti = function getDataMulti (req, res, next) {
  // default to now
  const toDate = requestUtils.parseTimeParameter(req, next, 'to-date', moment().utc());
  if (!moment.isMoment(toDate)) {
    return next(toDate);
  }

  // default to 15 days earlier
  const fromDate = requestUtils.parseTimeParameter(req, next, 'from-date', toDate.clone().subtract(15, 'days'));
  if (!moment.isMoment(fromDate)) {
    return next(fromDate);
  }
