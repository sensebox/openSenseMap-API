 ...
 * @apiUse BoxIdParam
 * @apiParam {Boolean=true,false} [luftdaten=false] signals the api to treat the incoming data as luftdaten.info formatted json.
 ...
 * sensorIDtheThird,value
 * ...
 * @apiParamExample {application/json} luftdaten=true:
 * {
 *   "sensordatavalues": [
 *     {
 *       "value_type": "SDS_P1",
...
const postNewMeasurements = function postNewMeasurements (req, res, next) {
  ...
  const handler = decodeHandlers[(
      req._userParams.luftdaten === 'true' // if
      ? 'luftdaten' // then
...
module.exports = {
  ...
  postNewMeasurements: [
    requestUtils.retrieveParameter('luftdaten', 'String', 'false', ['true', 'false']),
    postNewMeasurements
  ],
  postNewBox: [
    requestUtils.checkContentType,
