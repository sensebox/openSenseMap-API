...
const getDataMulti = function getDataMulti (req, res, next) {
  ...
  const delimiter = (req['delimiter'] === 'comma' ? ',' : ';'),
    columns = req['columns'];
  Box.findMeasurementsOfBoxesStream(queryParams, req['phenomenon'], req['from-date'].toDate(), req['to-date'].toDate(), columns)
...
module.exports = {
  ...
  getDataMulti: [
    ...
    requestUtils.retrieveParameter('bbox', 'Array,Number', false),
