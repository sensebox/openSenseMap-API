...
const getData = function getData (req, res) {
  ...
  const senseID = req._userParams.boxId;
  const sensor = req._userParams.sensorId;
  if (format === 'csv') {
    res.header('Content-Type', 'text/csv');
    const delimiter = (req._userParams.delimiter === 'comma' ? ',' : ';');
    ...
    var i = 0;
    const feed = new RSS({
      title: `SenseBox RSS-Feed (Date: ${stringDate} / ID: ${senseID} / Sensor: ${sensor})`,
      feed_url: `https://api.opensensemap.org/boxes/${senseID}/sensors/${sensor}?format=rss`,
      site_url: 'http://opensensemap.org',
      pubDate: stringDate,
    });
    stringifier = streamTransform(function (data) {
      feed.item({
        title: data.value,
        url: `https://api.opensensemap.org/boxes/${senseID}/sensors/${sensor}?format=rss`,
        guid: `${senseID}/${sensor}/${stringDate}/${i++}`,
        date: data.createdAt,
      });
...
const getDataMulti = function getDataMulti (req, res, next) {
  ...
  const delimiter = (req._userParams.delimiter === 'comma' ? ',' : ';'),
    columns = req._userParams.columns;
  Box.findMeasurementsOfBoxesStream(queryParams, req._userParams.phenomenon, req._userParams['from-date'].toDate(), req._userParams['to-date'].toDate(), columns)
...
const postNewMeasurement = function postNewMeasurement (req, res, next) {
  ...
  Box.findOne({ _id: req._userParams.boxId })
    .then(function (box) {
      if (!box) {
        return next(new restify.NotFoundError('no senseBox found'));
      }
...
module.exports = {
  ...
  getScript,
  getData: [
    requestUtils.retrieveParameter('format', 'String', 'json', ['json', 'csv', 'rss']),
    requestUtils.retrieveParameter('download', 'String', false, ['true', 'false']),
    requestUtils.retrieveParameter(['delimiter', 'separator'], 'String', false, ['comma']),
