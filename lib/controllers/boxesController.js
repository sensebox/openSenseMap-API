...
const findAllBoxes = function findAllBoxes (req, res, next) {
  const format = req._userParams.format,
    phenomenon = req._userParams.phenomenon,
    activityAroundDate = req._userParams.date;
  ...
  if (activityAroundDate && (dates = activityAroundDate.split(',')) && dates.length === 2 && moment(dates[0]).isBefore(dates[1])) { // moment().isBefore() will check the date's validities as well
    ...
    toDate = moment.utc(dates[1])
      .toDate();
  } else if (moment.utc(activityAroundDate).isValid()) {
    ...
    Promise.all(sensorQrys)
      ...
      .then(function (resultset) {
        if (format === 'json') {
          ...
          const features = [];
          for (const box of resultset) {
            const coordinates = box.loc[0].geometry.coordinates;
            box.loc = undefined;
            features.push(point(coordinates, box));
          }
          res.send(featureCollection(features));
        }
