...
const updateBox = function updateBox (req, res, next) {
  ...
  const { mqtt, loc, image, sensors, boxId, addons } = req._userParams;
  ...
  Box.findById(boxId).then(function (box) {
    ...
    if (mqtt) {
      ...
      box.set('mqtt', { enabled, url, topic, decodeOptions, connectionOptions, messageFormat });
    }
    ...
    box.save()
      .then(function (savedBox) {
        ...
        requestUtils.clearCache(['findAllBoxes']);
 ...
 */
const getData = function getData (req, res) {
  let stringifier;

  const { format, delimiter } = req._userParams;
  if (format === 'csv') {
...
const deleteBox = function deleteBox (req, res, next) {
  Box.deleteBox(req._userParams.boxId)
    .then(function () {
...
module.exports = {
  ...
  updateBox: [
    ...
    requestUtils.retrieveParameter('addons', 'object', false),
    updateBox
  ],
  getMeasurements,
  postNewMeasurement: [
