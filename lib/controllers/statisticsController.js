...
const idwHandler = function (req, res, next) {
  ...
  if (areaSqKm / req._userParams['cellWidth'] > 10000) {
    return next(new restify.UnprocessableEntityError('computation too expensive ((area in square kilometers / cellWidth) > 2500)'));
  }

  Box.findMeasurementsOfBoxesStream(queryParams, req._userParams['phenomenon'], req._userParams['from-date'].toDate(), req._userParams['to-date'].toDate(), idwColumns)
    .then(function (cursor) {

      const transformer = idwTransformer({
        bounds: queryParams['loc.geometry']['$geoWithin']['$geometry'],
        gridType: req._userParams['gridType'],
        cellWidth: req._userParams['cellWidth'],
        cellUnit: req._userParams['cellUnit'],
        power: req._userParams['power'],
        numClasses: req._userParams['numClasses']
      });
