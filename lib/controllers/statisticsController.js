...
const { idwTransformer, outlier } = statistics;
...
/**
 * @api {get} /statistics/outlier?boxid=57062bcb45fd40c81974667e&phenomenon=Temperatur Get data with outlier retrieved as json
 * @apiDescription Retrieve a JSON object containing required data with seperate outlier column with the calculated outlier if value doesn't fit in time series
 * The outliers are calculated within a moving window (default size 15). A value is categorized as outlier if it is not lying within three times median absolute
 * deviation around the median (calculated out of the previous values within the given window). For an outlier the running mean calculated out of the values within
 * the given window or true is retrieved (by default it is replaced by the mean, you can change it by setting replaceOutlier to 2).
 * @apiVersion 0.0.1
 * @apiGroup Outlier calculation
 * @apiName calculateOutlier
 * @apiParam {BoxID} BoxID the id of the box you want to download the data for.
 * @apiParam {String} phenomenon the name of the phenomenon you want to download the data for.
 * @apiParam {ISO8601Date} [from-date] Beginning date of measurement data (default: 2 days ago from now)
 * @apiParam {ISO8601Date} [to-date] End date of measurement data (default: now)
 * @apiParam {Number=1-50} [window=15] Size of moving window used as base to calculate the outliers
 * @apiParam {Number=1-2} [replaceOutlier=1] Define if the outlier should be replaced by the running mean, so set it 1, or just be retrieved as 'true', set it 2 (default: 1)
 */

const outlierHandler = function (req, res, next) {
  const { phenomenon, boxid, replaceOutlier, window, columns } = req._userParams,
    fromDate = req._userParams['from-date'],
    toDate = req._userParams['to-date'];

  // validate window
  if (window < 1 || window >= 50) {
    return next(new restify.BadRequestError('parameter window must be between 1 and 50, default is 15'));
  }

  // validate window
  if (replaceOutlier < 1 || replaceOutlier > 2) {
    return next(new restify.BadRequestError('replace must be either 1 for yes or 2 for no'));
  }
...
const GET_DATA_MULTI_DEFAULT_COLUMNS = ['createdAt', 'value', 'lat', 'lon'];
const GET_DATA_MULTI_ALLOWED_COLUMNS = ['createdAt', 'value', 'lat', 'lon', 'unit', 'boxId', 'sensorId', 'phenomenon', 'sensorType', 'boxName', 'exposure'];
...
module.exports = {
  ...
  getOutlier: [
    requestUtils.retrieveParameter('phenomenon', 'String', true),
    requestUtils.retrieveParameter('boxid', 'String', true),
    requestUtils.retrieveParameter('replaceOutlier', 'Number', 1, [1, 2]),
    requestUtils.retrieveParameter('window', 'Number', 15),
    requestUtils.retrieveParameter('columns', 'Array,String', GET_DATA_MULTI_DEFAULT_COLUMNS, GET_DATA_MULTI_ALLOWED_COLUMNS),
    requestUtils.validateBboxParam,
    requestUtils.parseAndValidateTimeParams,
    outlierHandler],
};
