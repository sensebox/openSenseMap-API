...
const handleUserError = function handleUserError (err, next) {
  if (err.name === 'ModelError' || ['Duplicate user detected', 'Password not correct.'].includes(err.message)) {
...
module.exports = {
  ...
  updateUser: [
    ...
    requestUtils.retrieveParameters([
      { name: 'email' },
      { name: 'currentPassword', dataType: 'as-is' },
      { name: 'newPassword', dataType: 'as-is' },
