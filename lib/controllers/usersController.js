...
const handleUserError = function handleUserError (err, next) {
  if (['Duplicate user detected', 'Password not correct.'].includes(err.message)) {
  ...
  req.user.resendEmailConfirmation()
    .then(function (result) {
      if (result.isConfirmed && result.isConfirmed === true) {
        return next(new restify.UnprocessableEntityError(`Email address ${req.user.email} is already confirmed.`));
      }
      let usedAddress = result.email;
      if (result.unconfirmedEmail && result.unconfirmedEmail !== '') {
