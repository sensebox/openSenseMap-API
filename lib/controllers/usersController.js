...
const updateUser = function updateUser (req, res, next) {
  const { email, name, language, currentPassword, newPassword } = req._userParams;
  ...
  p
    ...
    .catch(function (err) {
      if (err && err.errors && err.errors.unconfirmedEmail) {
        return next(new restify.UnprocessableEntityError(`An user with the email address ${email} already exists.`));
      }
