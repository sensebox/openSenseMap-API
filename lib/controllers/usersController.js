...
const { InternalServerError, UnprocessableEntityError } = restifyErrors;
  ...
  req.user.resendEmailConfirmation()
    .then(function (result) {
      if (result.isConfirmed && result.isConfirmed === true) {
        return next(new UnprocessableEntityError(`Email address ${req.user.email} is already confirmed.`));
      }
...
module.exports = {
  registerUser: [
    ...
    retrieveParameters([
      ...
      { name: 'password', required: true, dataType: 'as-is' },
  ...
  deleteUser: [
    checkContentType,
    retrieveParameters([
      { name: 'password', dataType: 'as-is', required: true }
    ]),
    deleteUser
  ]
};
