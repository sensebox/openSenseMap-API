    ...
    .then(function (newUser) {
      return newUser.createToken()
        .then(function (tokens) {
          return res.send(201, { code: 'Created', message: 'Successfully registered new user', data: { user: newUser }, token: tokens.jwt, refreshToken: tokens.refreshToken });
        })
        .catch(function (err) {
...
const signIn = function signIn (req, res, next) {
  req.user.createToken()
    .then(function (tokens) {
      return res.send(200, { code: 'Authorized', message: 'Successfully signed in', data: { user: req.user }, token: tokens.jwt, refreshToken: tokens.refreshToken });
    })
    .catch(function (err) {
...
const refreshJWT = function refreshJWT (req, res, next) {
  ...
  User.findOne({ refreshToken: token, refreshTokenExpires: { $gte: moment.utc().toDate() } })
    .then(function (user) {
      if (!user) {
        return next(new restify.ForbiddenError('Refresh token invalid or too old. Please sign in with your username and password.'));
      }

      // invalidate old token
      tokenBlacklist.addTokenHashToBlacklist(token);

      return user.createToken()
        .then(function (tokens) {
          return res.send(200, { code: 'Authorized', message: 'Successfully refreshed auth', data: { user }, token: tokens.jwt, refreshToken: tokens.refreshToken });
        })
        .catch(function (err) {
          next(new restify.InternalServerError(`unable to create jwt token: ${err.message}`));
        });
...
const resetPassword = function resetPassword (req, res, next) {
  const { password, token } = req._userParams;
  ...
  p
    ...
    .catch(function (err) {
      if (err && err.errors && err.errors.unconfirmedEmail) {
        return next(new restify.UnprocessableEntityError(`New email address invalid or an user with the email address ${email} already exists.`));
      }
